...
vi.mock('../../src/utils/background-service.js', () => ({
	...
	unregisterPushTokenForAccount: vi.fn(() => Promise.resolve()),
}));
...
describe('push-notifications safety guards', () => {
	...
	describe('cleanup', () => {
		it('deletes the persisted server registration ID', async () => {
			const registrations = { '__active_session__': { regId: 'registration-123', token: 'tok', platform: 'fcm' } };
			localStorageState.set(REGISTRATIONS_KEY, JSON.stringify(registrations));
			...
			expect(localRemove).toHaveBeenCalledWith(REGISTRATIONS_KEY);
		});
