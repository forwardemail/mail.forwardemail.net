...
function seedCurrentRegistration({
	...
	// Per-account registration storage (new model)
	const registrations = { 'user@example.com': { regId: id, token, platform } };
	localStore.set('push_registrations', JSON.stringify(registrations));
}
...
describe('push notification status and management', () => {
	...
	it('deregisters the current device locally and through the owned server resource', async () => {
		...
		// Per-account registrations should be cleared for this account
		const regs = JSON.parse(localStore.get('push_registrations') || '{}');
		expect(regs['user@example.com']).toBeUndefined();
	});
	...
	it('re-registers by removing the old server resource before creating the replacement', async () => {
		...
		expect(unregisterServerMock).toHaveBeenCalledWith('registration-1');
		expect(registerServerMock).toHaveBeenCalledWith(FCM_TOKEN, 'android');
		expect(unregisterServerMock.mock.invocationCallOrder[0]).toBeLessThan(registerServerMock.mock.invocationCallOrder[0]);
	});
	...
	it('removes a selected stale server registration without deregistering this device', async () => {
		...
		// Active account's registration should still be intact
		const regs = JSON.parse(localStore.get('push_registrations') || '{}');
		expect(regs['user@example.com']?.regId).toBe('registration-1');
	});
	...
	it('returns registration-timeout when native token retrieval hangs', async () => {
		localStore.set('alias_auth', ALIAS_AUTH);
		fcmGetTokenMock.mockImplementationOnce(() => new Promise(() => {}), // Never resolves
		);
