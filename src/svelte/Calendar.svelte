<script lang="ts">
  ...
  const effectiveSelectedCalendarIds = $derived(
    isMobile
      ? (calendars.map((c) => getCalendarId(c)).filter(Boolean) as string[])
      : selectedCalendarIds,
  );
  ...
  const getCalendarLabel = (cal: unknown) => {
    const rawName =
      (cal as Record<string, unknown>)?.name || (cal as Record<string, unknown>)?.displayName || '';
  ...
  const buildInviteMessage = (eventDetails: {
    title?: string;
    start: Date;
    end: Date;
    allDay?: boolean;
  ...
  const getFocusableElements = (container: HTMLElement | null) => {
    ...
    return Array.from(focusables).filter(
      (el) =>
        ...
        ((el as HTMLElement).offsetParent !== null || el === document.activeElement),
    );
  };
  ...
  const importICS = async (eventOrFiles: Event | File[]) => {
    ...
    try {
      ...
      for (const eventBlock of eventBlocks) {
        ...
        const existing = rawUid
          ? events.find(
              (ev) =>
                (ev as Record<string, unknown>).id === rawUid ||
                (ev as Record<string, unknown>).uid === rawUid,
            )
          : null;
      ...
      toasts?.show?.(
        'Failed to import calendar: ' + ((err as Error)?.message || 'Unknown error'),
        'error',
      );
    } finally {
      if (target) target.value = '';
  ...
  const reconcileCalendarSelection = (
    list: unknown[],
    prefs: { selectedIds: string[]; activeId: string } | null,
  ) => {
    ...
    if (activeId && !selectedIds.includes(activeId))
      selectedIds = uniqueIds([...selectedIds, activeId]);
  ...
  const fetchCalendars = async (attempt = 1): Promise<void> => {
    ...
    if (cached?.value && (cached.value as unknown[])?.length) {
      ...
      const prefsSnapshot = selectedCalendarIds.length
        ...
        : storedPrefs;
      const { selectedIds, activeId } = reconcileCalendarSelection(
  ...
  const setCalendarSelection = async (
    nextSelectedIds: string[],
    nextActiveId: string,
    options: { reloadEvents?: boolean } = {},
  ) => {
    ...
    const finalSelected =
      activeId && !cleaned.includes(activeId) ? uniqueIds([...cleaned, activeId]) : cleaned;
  ...
  const toggleCalendarSelection = async (calendarId: string) => {
    ...
    const nextActive = nextSelected.includes(activeCalendarId) ? activeCalendarId : nextSelected[0];
